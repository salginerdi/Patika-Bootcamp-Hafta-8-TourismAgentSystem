...
import com.TurizmAcentaSistemi.Model.ResInfo;
import com.TurizmAcentaSistemi.Model.Room;
import com.TurizmAcentaSistemi.Model.RoomProp;
...
public class ReservationGUI extends JFrame {
    ...
    public ReservationGUI(Room room, int adult_numb, int child_numb, String check_in, String check_out){
        ...
        RoomProp roomProperty = RoomProp.getFetch(room.getId());
        ...
        // 16 - Acenta çalışanı müşterinin talebine uygun odayı müşteri bilgilerini girerek rezervasyon yaptığı bölüm.
        // 16 - The section where the agency employee makes a reservation by entering the customer's information about the room suitable for the customer's request.
        btn_add_reservation.addActionListener(e -> {
            if (Helper.isFieldEmpty(fld_client_name) || Helper.isFieldEmpty(fld_client_phone) || Helper.isFieldEmpty(fld_client_mail) || Helper.isAreaEmpty(txtArea_client_note)){
                ...
                // 17 - Rezervasyon yapılan odanın stoğu azalıyor.
                // 17 - The stock of the booked room is decreasing.
                if (ResInfo.add(client_name, client_phone, client_email, client_note, room_id, check_in, check_out, adult_numb, child_numb, total_price)){
                    Helper.showMsg("Rezervasyon işlemi başarıyla yapılmıştır!");
